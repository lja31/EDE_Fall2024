...
#Save the plot
ggsave(
  filename = "/home/guest/EDE_Fall2024/OUTPUT/Plot_text.jpg",
  plot = Plot_text, 
  height = 4, 
  width = 6, 
  units = "in", 
...
ggplot(PeterPaul.chem.nutrients) + 
  geom_bar(aes(x = month , y = lakename))
...
PeterPaul.chem.nutrients %>% 
  ggplot()+geom_bar (aes(x = month )) +scale_x_discrete(drop=F)
...
surface_temp_plot3 <- PeterPaul.chem.nutrients %>% 
  filter(depth == 0 ) %>% 
  ggplot(
    mapping = aes(
      x=daynum,
...
surface_temp_plot4 <- PeterPaul.chem.nutrients %>% 
  ggplot(
    mapping = aes(
      x=sampledate,
      y=temperature_C,
