...
#Build a function to fix year that is more general than the one discussed in the lesson #cutoff another date object
fix.early.dates <- function(d, cutoff) {
  
       m <- year(d) %% 100  
       # The %% operator does modular division i.e. integer-divides and return reminder 
...
#Alternative 1
date_obj_past <- fast_strptime(str_past,"%y-%b-%d",cutoff_2000=24L)#L for long integer 
date_obj_past
...
usgs_flow_data <- read.csv(
  file = here('Data','Raw','USGS_Site02085000_Flow_Raw.csv'),
  stringsAsFactors = T,
  colClasses = (c('site_no'='factor')) #convert numbers
)
