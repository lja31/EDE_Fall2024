...
ggplot(BR_complete, aes(x = Date, y = Rainfall)) +
  geom_line() + geom_smooth(method="lm")  #seasonality is clear
...
BR_complete_Rainfall_ts <- ts(BR_complete$Rainfall, start = c(f_year, f_month), frequency = 12)
BR_complete_Temperature_ts <- ts(BR_complete$Temperature_C, start = c(f_year, f_month), frequency = 12)
...
BR_complete_Rainfall_Decomposed <- stl(BR_complete_Rainfall_ts, s.window = "periodic")
BR_complete_Temperature_Decomposed <- stl(BR_complete_Temperature_ts, s.window = "periodic")


# Visualize the decomposed series. 
plot(BR_complete_Rainfall_Decomposed)
plot(BR_complete_Temperature_Decomposed)
...
Br_complete_rain_Components <- as.data.frame(BR_complete_Rainfall_Decomposed$time.series[,1:3])
Br_complete_temp_Components <- as.data.frame(BR_complete_Temperature_Decomposed$time.series[,1:3])

Br_complete_rain_Components <- mutate(Br_complete_rain_Components,
        Observed = BR_complete$Rainfall,     
        Date = BR_complete$Date)


Br_complete_temp_Components <- mutate(Br_complete_temp_Components,
...
ggplot(Br_complete_rain_Components) +
...
ggplot(Br_complete_temp_Components) +
  geom_line(aes(y = Observed, x = Date),  size = 0.25) +
