...
NT.phys.data.1999 <- filter(NTL.phys.data, year4==1999)
dim(NT.phys.data.1999)
# Exercise 2: 
# filter NTL.phys.data for Tuesday Lake from 1990 through 1999.

NTL.phys.data.Tuesday <- 
  filter(NTL.phys.data, lakename %in% ("Tuesday Lake"), year4 %in% c(1990:1999))
dim(NTL.phys.data.Tuesday)
```
Question: Why don't we filter using row numbers?

> Answer: not in our control. we don't control or filter on how data is organized.
...
NTL.phys.data.Tuesday.July <- 
    NTL.phys.data.Tuesday %>%
  filter(lakename %in% ("Tuesday Lake") & year4 %in% c(1990:1999) & month(sampledate)%in% 7)
...
NTL.phys.data.Tuesday.temp <- 
  NTL.phys.data.Tuesday.July %>%
filter(depth %in% 0.25) %>%
