...
```{r Create boxplots}
#5
#all_months <- data.frame(month = unique(NTL.LTER.Lake.Chemistry.Nutrients.PeterPaul$month))
# Here I merge my original data with the data frame containing all months
#NTL.LTER.Lake.Chemistry.Nutrients.PeterPaul <- merge(all_months, #NTL.LTER.Lake.Chemistry.Nutrients.PeterPaul, by = "month", all.x = TRUE)
...
plot_temp <- NTL.LTER.Lake.Chemistry.Nutrients.PeterPaul %>% 
  ggplot(
    aes(x=factor(month, levels = unique(month)), y = temperature_C,
        fill = lakename)) +
  geom_boxplot() +
...
plot_tn <- NTL.LTER.Lake.Chemistry.Nutrients.PeterPaul %>% 
  ggplot(aes(x = factor(month, levels = unique(month)),
