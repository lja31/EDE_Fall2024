...
summary_PM25 <- epa_data %>% 
  ...
  arrange(mean_PM2.5)
kable(x=summary_PM25, caption = "Mean Particulates (2.5mm)", 
...
summary_Ozone <- epa_data %>% 
  ...
  arrange(mean_Ozone)
kable(x=summary_Ozone, caption = "Mean Ozone", 
...
```{r, echo=FALSE, warning=FALSE, fig.cap="2018 monthly Ozone", fig.dim=c(7,3)}
library(ggridges)
...
Ozonedataset1 <- epa_data %>% 
  filter(Year == 2018) %>% 
  ggplot(aes(x = Month, y=Ozone)) +
  geom_boxplot() +
labs(title = "Monthly Ozone concentrations (2018)", x="Month", 
...
```{r, echo=FALSE, warning=FALSE, fig.cap="2019 monthly Ozone", fig.dim=c(7,3)}
Ozonedataset2 <- epa_data %>% 
  filter(Year == 2019) %>% 
  ggplot(aes(x = Month, y=Ozone)) +
  geom_boxplot() +
labs(title = "Monthly Ozone concentrations (2019)", x="Month", 
     y = expression(paste("Concentration (", mu, "g/m^3)"))) +
scale_x_discrete(expand = c(0, 0), drop=FALSE) +
  theme(
        legend.position = "up"
